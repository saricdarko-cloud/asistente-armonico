      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (formData.modulation === 'modulate') {
          ...
          switch(formData.modulationDistance) {
            case 'vecinas': interval = [7, 5, 9, 4][Math.floor(Math.random()*4)]; targetQuality = (interval === 9 || interval === 4) ? 'm' : ''; explanation += `Para esta modulación cercana, nos dirigimos a la región de ${transpose(rootIndex, interval)}. `; break;
            case 'homonimas': targetQuality = isInputMinor ? '' : 'm'; explanation += `Aplicando un intercambio modal paralelo, cambiamos la cualidad del centro tonal. `; break;
            case 'mediantes': interval = [4, 8, 3, 9][Math.floor(Math.random()*4)]; targetQuality = (Math.random() > 0.5) ? 'm' : ''; explanation += `Utilizamos una mediante cromática para saltar armónicamente hacia ${transpose(rootIndex, interval)}${targetQuality}. `; break;
            case 'lejanas': interval = [2, 10][Math.floor(Math.random()*2)]; explanation += `Modulamos hacia una tonalidad lejana (${transpose(rootIndex, interval)}). `; break;
            case 'muy_lejanas': interval = 6; explanation += `Realizamos una modulación extrema a distancia de tritono (${transpose(rootIndex, interval)}). `; break;
            case 'sorpresa': interval = Math.floor(Math.random() * 11) + 1; targetQuality = (Math.random() > 0.5) ? 'm' : ''; explanation += `He calculado una modulación inesperada hacia ${transpose(rootIndex, interval)}${targetQuality}. `; break;
          }
        ...
        let route: string[] = [];
        let subQuality = useExtensions ? 'maj9' : 'maj7'; let domQuality = useExtensions ? '13' : '7'; let minQuality = useExtensions ? 'm9' : 'm7'; let halfDim = useExtensions ? 'm11b5' : 'm7b5';

        if (formData.mood === 'tenso' || formData.mood === 'misterioso') { domQuality = '7#9'; subQuality = 'm(maj7)'; } 
        else if (formData.mood === 'etereo' || formData.mood === 'intimo') { domQuality = '9sus4'; subQuality = 'add9'; } 
        else if (formData.mood === 'epico' || formData.mood === 'triunfal') { domQuality = 'sus4'; subQuality = ''; }

        const approachType = i % 3; 

        if (approachType === 0) {
          let prepChordIndex = (targetIndex + 2) % 12; let domChordIndex = (targetIndex + 7) % 12;
          if (targetQuality === 'm') { route.push(`${transpose(prepChordIndex, 0)}${halfDim}`); route.push(`${transpose(domChordIndex, 0)}${domQuality.includes('b9') ? domQuality : '7b9'}`); } 
          else { route.push(`${transpose(prepChordIndex, 0)}${minQuality}`); route.push(`${transpose(domChordIndex, 0)}${domQuality}`); }
          ...
          let subVIndex = (targetIndex + 1) % 12; let prepIndex = (targetIndex + 5) % 12;
          route.push(`${transpose(prepIndex, 0)}${targetQuality === 'm' ? 'm7' : subQuality}`); route.push(`${transpose(subVIndex, 0)}${useExtensions ? '7#11' : '7'}`); route.push(`${transpose(targetIndex, 0)}${targetQuality === 'm' ? minQuality : subQuality}`);
          explanation += `Empleamos una sustitución tritonal para un color más sofisticado de ${formData.style}. `;
        } else {
          let bVIIndex = (targetIndex + 8) % 12; let bVIIIndex = (targetIndex + 10) % 12;
          route.push(`${transpose(bVIIndex, 0)}maj7`); route.push(`${transpose(bVIIIndex, 0)}${domQuality}`); route.push(`${transpose(targetIndex, 0)}${targetQuality === 'm' ? minQuality : subQuality}`);
